...
"""
# Вам потрібно написати філософію Python у форматі рядка:
# 1 знайти окремо кількість входжень слів "better", "never" і "is" в даному рядку
# 2 потрібно виводити весь текст у верхньому регістрі (всі літери у верхньому регістрі)
# 3 замінити всі входження символу «i» на «&»

# (1)
# words_total = CONST_ZEN.split()
words_total = CONST_ZEN.replace('.',' ').replace(',',' ').replace('!',' ')
words_total = str(words_total.split())
# print(words_total)
words_better = words_total.count('better')
words_never = words_total.count('never')
words_is = words_total.count('is')
print("Total words (better):", words_better, "\nTotal words (never):", words_never, "\nTotal words (is):", words_is)

# (2)
print(CONST_ZEN.upper())

# (3)
print(CONST_ZEN.replace('i','&').replace('I','&'))